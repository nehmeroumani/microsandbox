...
use std::{collections::BTreeMap, fs::File, io, sync::RwLock};

use super::file_ops::{file_read_at, file_write_at};
use super::{NodeKind, PathFs, VAttr, VDirEntry, parent_path};
// Linux wire errno values from the crate's single errno table.
pub(crate) use crate::backends::shared::platform::{
    LINUX_EEXIST, LINUX_EINVAL, LINUX_EISDIR, LINUX_ENODATA, LINUX_ENOENT, LINUX_ENOTDIR,
};
...
impl PathFs for InMemoryFs {
    ...
    fn readdir(&self, path: &[u8]) -> io::Result<Vec<VDirEntry>> {
        ...
        match map.get(&k) {
            ...
            Some(_) => return Err(err(LINUX_ENOTDIR)),
    ...
    fn readlink(&self, path: &[u8]) -> io::Result<Vec<u8>> {
        ...
        if node.kind != NodeKind::Symlink {
            return Err(err(LINUX_EINVAL));
        }
        Ok(node.target.clone())
    }

    fn setxattr(&self, path: &[u8], name: &[u8], value: &[u8], _flags: u32) -> io::Result<()> {
        let mut map = self.map.write().unwrap();
        let node = map.get_mut(&key(path)).ok_or_else(|| err(LINUX_ENOENT))?;
...
impl ZeroCopyWriter for MockWriter {
    fn write_from(&mut self, f: &File, count: usize, off: u64) -> io::Result<usize> {
        ...
        let n = file_read_at(f, &mut tmp, off)?;
...
impl ZeroCopyReader for MockReader {
    fn read_to(&mut self, f: &File, count: usize, off: u64) -> io::Result<usize> {
        ...
        let n = file_write_at(f, &remaining[..to_write], off)?;
        self.pos += n;
        Ok(n)
    }
}
