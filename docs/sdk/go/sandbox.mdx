...
<div className="msb-glance">
  ...
  <div className="msb-chiprow">
    ...
    <a className="msb-chip" href="#withpullprogress">WithPullProgress()</a>
...
Create the sandbox in detached mode. The VM continues running after the Go process exits; reattach via [`GetSandbox`](#m-getsandbox). Note that [`Close`](#sb-close) stops a detached sandbox, use [`Detach`](#sb-detach) to leave it running. Cannot be combined with [`WithVirtualMount`](#withvirtualmount): the mount's provider runs in this process and would not survive it, so the combination is rejected at create.
...
Mount a programmable filesystem at `guestPath`, served by a `vfs.PathFs` provider implemented in your Go code. Reads and writes the guest performs under the path become calls into the provider. Not usable with [`WithDetached`](#withdetached) — the provider runs in this process, so the mount only lives while the Sandbox is open. See [Virtual Mounts](/sdk/go/virtual-mounts).
...
<div className="msb-params">
  ...
  <div className="msb-param">
    <div className="msb-param-key"><code>provider</code><a className="msb-type" href="/sdk/go/virtual-mounts#the-pathfs-interface">vfs.PathFs</a></div>
...
#### <span className="msb-recv"></span><span className="msb-hn">WithPullProgress()</span>
<div className="msb-tags"><span className="msb-tag is-builder">option</span></div>
...
Register a callback invoked with per-layer image download and extraction progress while the sandbox's image is pulled during [`CreateSandbox`](#m-createsandbox). Has no effect when the image is already cached locally (no pull happens). The callback runs synchronously on the creating goroutine, in event order — keep it fast and do not call back into the sandbox from inside it.

<p className="msb-label">Parameters</p>

<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>fn</code><span className="msb-type">func(PullProgress)</span></div>
    <div className="msb-param-desc">Receives one <code>PullProgress</code> event per phase update.</div>
  </div>
</div>

Each `PullProgress` event carries a `Kind` identifying the phase, plus the fields that phase populates:

| Kind | Meaning | Key fields |
|------|---------|-----------|
| `PullResolving` / `PullResolved` | Reference resolution | `Reference`, `ManifestDigest`, `LayerCount`, `TotalDownloadBytes` |
| `PullLayerDownloadProgress` / `…Complete` / `…Verifying` | Per-layer download | `LayerIndex`, `Digest`, `DownloadedBytes`, `TotalBytes` |
| `PullLayerMaterializeStarted` / `…Progress` / `…Writing` / `…Complete` | Per-layer extraction | `LayerIndex`, `DiffID`, `BytesRead`, `TotalBytes` |
| `PullStitchMergingTrees` / `…WritingFsmeta` / `…WritingVMDK` / `…Complete` | Image assembly | `LayerCount` |
| `PullComplete` | Pull finished | `Reference`, `LayerCount` |

`TotalDownloadBytes` and `TotalBytes` are `*uint64` and nil when the registry manifest omits sizes.
...
sb, err := m.CreateSandbox(ctx, "demo",
    m.WithImage("python:3.12"),
    m.WithPullProgress(func(ev m.PullProgress) {
        switch ev.Kind {
        case m.PullLayerDownloadProgress:
            fmt.Printf("layer %d: %d bytes\n", ev.LayerIndex, ev.DownloadedBytes)
        case m.PullComplete:
